...
export class RatingService {
  async submitRating(raterPublicId: string, dto: SubmitRatingDto): Promise<IRating> {
    ...
    if (cls.status !== ClassStatus.COMPLETED) {
      throw new AppError('Class must be completed before rating', 400);
    }

    // cls.studentPublicId is the STUDENT PROFILE id; raterPublicId is the USER id.
    // Resolve the rater's student profile and compare profile ids.
    const studentProfile = await StudentProfileModel.findOne(
      { userPublicId: raterPublicId, isDeleted: false }, { publicId: 1 },
    ).lean();
    if (!studentProfile || cls.studentPublicId !== studentProfile.publicId) {
      throw new AppError('Only the student of this class can submit a rating', 403);
    }

    const existing = await RatingModel.findOne({ classPublicId: dto.classPublicId, raterPublicId });
    if (existing) throw new AppError('You have already rated this class', 409);
