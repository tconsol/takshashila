import React, {
  ...
  useEffect, useMemo, useRef, type ReactElement, type HTMLAttributes,
} from 'react';
...
const CardSwap = ({
  ...
  useEffect(() => {
    ...
    // Reset order + place every card at its slot (clean state on (re)mount/HMR)
    ...
    const swap = () => {
      ...
      tl.to(elFront, { y: '+=180', duration: config.durDrop, ease: config.ease });
    ...
    // First swap after one delay (no jarring drop on mount)
  ...
  const rendered = childArr.map((child, i) =>
    isValidElement(child)
      ? cloneElement(child as ReactElement<CardProps & { ref?: React.Ref<HTMLDivElement> }>, {
          key: i,
          ref: refs[i],
