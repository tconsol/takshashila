...
export function ChatPage() {
  ...
  const otherRole = selected
    ? (selected.participantRoles.find(
        (_, i) => selected.participantPublicIds[i] !== user?.publicId,
      ) ?? '')
    : '';

  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm">
      ...
      <div className="flex-1">
        {activeId ? (
          <ChatWindow conversationPublicId={activeId} otherRole={otherRole} />
        ) : (
