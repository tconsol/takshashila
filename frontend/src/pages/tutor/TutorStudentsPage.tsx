  ...
  return (
    <div className="space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {completedClasses.map((cls) => (
              ...
              >
                <Avatar name={cls.studentPublicId.slice(0, 6)} size="sm" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-800 dark:text-gray-200 truncate">
                    {cls.subject}
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {format(new Date(cls.scheduledStartUTC), 'MMM d, yyyy')}
                  </p>
                </div>
      ...
      <Modal
        ...
        title={selectedClass ? `Attendance ${selectedClass.subject}` : ''}
        size="lg"
      >
        {selectedClass && (
          <div className="space-y-3">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {format(new Date(selectedClass.scheduledStartUTC), 'EEE, MMM d yyyy · h:mm a')}
            </p>
            <AttendanceSheet
