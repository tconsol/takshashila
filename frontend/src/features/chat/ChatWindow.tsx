...
function PinModal({ onConfirm, onCancel }: { onConfirm: (hours: number) => void; onCancel: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-[24px] border-2.5 border-clay-ink bg-white shadow-clay p-6">
        <h3 className="text-base font-extrabold text-clay-ink mb-1">Choose how long your pin lasts</h3>
        <p className="text-xs font-semibold text-clay-ink/50 mb-5">You can unpin at any time.</p>
            ...
            <label key={opt.hours} className="flex items-center gap-3 cursor-pointer">
              <div
                ...
                className={`h-5 w-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                  choice === opt.hours ? 'border-clay-green bg-white' : 'border-clay-ink/30 bg-white'
                }`}
              >
                {choice === opt.hours && <div className="h-2.5 w-2.5 rounded-full bg-clay-green" />}
              </div>
              <span className="text-sm font-extrabold text-clay-ink">{opt.label}</span>
            </label>
          ))}
        </div>
        <div className="flex justify-end gap-3">
          <button onClick={onCancel} className="px-4 py-2 text-sm font-extrabold text-clay-green hover:text-clay-green-dark transition-colors">
            Cancel
          </button>
          <button
            onClick={() => onConfirm(choice)}
            className="px-5 py-2 rounded-2xl border-2.5 border-clay-ink bg-clay-green text-white font-extrabold text-sm shadow-clay-sm hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-clay-pressed transition-all"
          >
            Pin
          </button>
        </div>
...
export function ChatWindow({ conversationPublicId, otherName, otherRole }: Props) {
  ...
  const [pinTarget, setPinTarget] = useState<IMessage | null>(null); // message waiting for modal
  ...
  useEffect(() => {
    ...
    setAllMessages(msgs);
    // Restore pinned message from loaded data
    const now = new Date();
  ...
  async function sendVoiceMessage() {
    ...
    try {
      // Strip codec suffix (e.g. "audio/webm;codecs=opus" → "audio/webm") so server MIME allowlist matches
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-shrink-0 flex items-center gap-3 px-4 py-3 border-b-2.5 border-clay-ink bg-clay-mint">
        ...
        <div className="min-w-0">
          ...
          {typingName && (
            <p className="text-xs font-bold text-clay-green-dark animate-pulse leading-none mt-0.5">{typingName} is typing…</p>
          )}
        </div>
      </div>
      ...
      {pinnedMessage && (
        <div className="group flex-shrink-0 flex items-center gap-2 px-4 py-2 border-b border-clay-ink/10 bg-white">
          <Pin className="h-3.5 w-3.5 text-clay-green flex-shrink-0" />
          <p className="flex-1 text-xs font-extrabold text-clay-ink truncate">
          ...
          <button
            ...
            className="flex-shrink-0 hidden group-hover:flex items-center gap-1 px-2.5 py-1 rounded-xl border border-clay-ink/20 bg-clay-bg text-[10px] font-extrabold text-clay-ink hover:bg-clay-coral/30 transition-colors"
          >
            ...
            <div key={msg.publicId}>
              {showDateSep && (
                <div className="flex items-center justify-center my-3">
                  <span className="px-3 py-0.5 rounded-full bg-gray-200 text-[11px] font-medium text-gray-500">
      ...
      {/* Selection mode bar */}
      {isSelectionMode && (
        <div className="flex-shrink-0 flex items-center justify-between gap-3 px-4 py-2.5 bg-clay-ink text-white border-t-2.5 border-clay-ink">
          <div className="flex items-center gap-3">
            <button onClick={handleExitSelectionMode} className="flex h-7 w-7 items-center justify-center rounded-full border-2 border-white/30 hover:bg-white/10 transition-colors">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl border-2 border-rose-400/50 bg-rose-500/20 text-rose-300 font-extrabold text-xs hover:bg-rose-500/30 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
      ...
      {/* Undo toast */}
      {pendingDelete && (
        <div className="flex-shrink-0 flex items-center justify-between gap-3 px-4 py-2.5 bg-clay-ink text-white text-xs font-extrabold">
          <span>Message deleted</span>
          <button onClick={handleUndo} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border-2 border-white/30 bg-white/15 hover:bg-white/25 transition-colors">
      ...
      <div className="flex-shrink-0 bg-white border-t-2.5 border-clay-ink">
        ...
        {/* Reply preview */}
        {replyTo && (
          <div className="flex items-center gap-2 px-4 pt-2.5 pb-1 border-b border-clay-ink/10">
            <div className="flex-1 min-w-0 border-l-4 border-clay-green pl-2">
              <p className="text-[10px] font-extrabold text-clay-green">{replyTo.sender}</p>
              <p className="text-xs font-semibold text-clay-ink/70 truncate">{replyTo.body}</p>
            </div>
            <button onClick={() => setReplyTo(null)} className="flex-shrink-0 h-6 w-6 flex items-center justify-center rounded-full hover:bg-clay-bg transition-colors">
              <X className="h-3.5 w-3.5 text-clay-ink/50" />
            </button>
          </div>
        )}
        ...
        {pendingFile && (
          <div className="flex items-center gap-2 px-4 pt-2.5 pb-1">
            <div className="flex items-center gap-2 flex-1 min-w-0 border-2 border-clay-ink bg-clay-mint rounded-xl px-3 py-2">
              <span className="text-clay-ink flex-shrink-0">{fileIcon(pendingFile.type)}</span>
        ...
        {fileError && <p className="px-4 pt-1.5 text-xs font-bold text-rose-600">{fileError}</p>}

        {/* Voice recording bar */}
        {isVoiceMode ? (
          <div className="flex items-center gap-3 px-4 py-3 bg-clay-ink">
            {/* Delete */}
            <button onClick={cancelRecording} className="flex-shrink-0 h-10 w-10 flex items-center justify-center rounded-2xl border-2 border-white/20 bg-white/10 text-white hover:bg-rose-500/30 transition-colors">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="text-white font-extrabold text-sm tabular-nums">{formatSeconds(recordingSecs)}</span>
            </div>
            ...
            <button
              ...
              className="flex-shrink-0 h-10 w-10 flex items-center justify-center rounded-2xl border-2 border-white/20 bg-white/10 text-white hover:bg-white/20 transition-colors"
            >
            ...
            <button
              ...
              disabled={uploading}
              className="flex-shrink-0 h-10 w-10 flex items-center justify-center rounded-2xl border-2 border-clay-green bg-clay-green text-white hover:translate-x-[1px] hover:translate-y-[1px] transition-all disabled:opacity-40"
            >
          ...
          <div className="flex items-end gap-2 px-4 py-3">
            ...
            <div ref={emojiPickerRef} className="relative flex-shrink-0">
              <button
                ...
                className="h-11 w-11 flex items-center justify-center rounded-2xl border-2 border-clay-ink bg-clay-yellow text-clay-ink hover:translate-x-[1px] hover:translate-y-[1px] transition-all"
              >
            ...
            <button
              ...
              disabled={uploading}
              className="flex-shrink-0 h-11 w-11 flex items-center justify-center rounded-2xl border-2 border-clay-ink bg-clay-sky text-clay-ink hover:translate-x-[1px] hover:translate-y-[1px] transition-all disabled:opacity-40"
            >
            ...
            {!input.trim() && !pendingFile ? (
              <button
                ...
                className="flex-shrink-0 h-11 w-11 flex items-center justify-center rounded-2xl border-2 border-clay-ink bg-clay-mint text-clay-ink hover:translate-x-[1px] hover:translate-y-[1px] transition-all"
              >
              ...
              <button
                onClick={handleSend}
                disabled={!canSend}
                className="flex-shrink-0 h-11 w-11 flex items-center justify-center rounded-2xl border-2 border-clay-ink bg-clay-green text-white hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-clay-pressed disabled:opacity-40 disabled:cursor-not-allowed transition-all"
              >
