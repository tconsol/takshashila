...
const WORDS = [
  ...
  { word: 'THUNDER', hint: '⛈️ Loud storm sound' },
...
function scramble(word: string): string {
  ...
  let scrambled = word;
  while (scrambled === word) scrambled = arr.sort(() => Math.random() - 0.5).join('');
  return scrambled;
}
...
export function WordScrambleGame() {
  ...
  const pick = (i: number) => {
    ...
    if (newTyped.length === item.word.length) {
      const correct = newTyped.join('') === item.word;
    ...
    return (
      <div className="flex flex-col items-center gap-5 py-10 text-center">
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 200 }}>
          ...
          <h2 className="text-4xl font-black text-white">All Done!</h2>
          <p className="text-3xl font-black text-yellow-300 mt-2">
            {score}/{WORDS.length * 10} pts
          </p>
        </motion.div>
        <motion.button
          onClick={() => {
  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full relative">
      ...
      <div className="flex items-center justify-between w-full max-w-md text-sm font-bold text-white">
        <span className="rounded-xl bg-white/10 border border-white/10 px-3 py-1.5">Q {qIdx + 1}/{WORDS.length}</span>
      ...
      >
        <p className="text-blue-200 text-base font-bold">{item.hint}</p>
        <p className="text-white/50 text-xs mt-1">{item.word.length} letters</p>
      </motion.div>
          ...
          <motion.button
            ...
            className={`w-12 h-12 rounded-xl text-xl font-black transition-all
              ${l.used
                ...
                : 'bg-gradient-to-br from-indigo-600 to-purple-700 hover:from-indigo-500 hover:to-purple-600 text-white shadow-lg cursor-pointer'}`}
            style={!l.used ? { boxShadow: '0 4px 16px rgba(99, 102, 241, 0.4)' } : {}}
          >
            {l.ch}
          </motion.button>
        ))}
      ...
      {showHint && <p className="text-yellow-300 font-bold text-sm">Starts with: {item.word[0]}</p>}
    </div>
  );
}
